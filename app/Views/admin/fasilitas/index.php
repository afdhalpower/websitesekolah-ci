...
<div class="page-header-modern mb-4">
  <div>
    <h1 class="page-title"><i class="fas fa-building"></i> Fasilitas</h1>
...
<!-- Bulk Action Form -->
<?= form_open(base_url('admin/fasilitas/proses')) ?>
        ...
        <tbody>
          ...
          <tr>
            <td class="text-center">
              <input type="checkbox" name="id_fasilitas[]" value="<?= esc($f->id_fasilitas) ?>"
                     class="row-check" onchange="updateCount()">
            </td>
            <td class="text-center"><?= esc($no) ?></td>
            <td>
              <?php if($f->gambar != ''): ?>
                <img src="<?= base_url('assets/upload/image/thumbs/'.$f->gambar) ?>" style="width:50px;height:40px;object-fit:cover;border-radius:6px;" alt="">
              <?php else: ?>
            ...
            <td>
              <a href="<?= base_url('admin/fasilitas/edit/'.$f->id_fasilitas) ?>" style="font-weight:500;color:var(--text);text-decoration:none;">
                <?= esc($f->judul_fasilitas) ?>
              </a>
              <div style="font-size:var(--font-xs);color:var(--muted);margin-top:2px;">
                <i class="fas fa-barcode"></i> <?= esc($f->kode_nomor_fasilitas) ?>
                &nbsp;<i class="fas fa-calendar"></i> <?= esc($f->tahun_fasilitas) ?>
              </div>
            </td>
            <td>
              <span class="status-badge status-info" style="font-size:var(--font-xs);">
                <?= esc($f->nama_kategori_fasilitas) ?>
              </span>
              <div style="font-size:var(--font-xs);color:var(--muted);margin-top:2px;">
                <i class="fas fa-check-circle"></i> <?= esc($f->kondisi_fasilitas) ?>
              </div>
            </td>
            <td>
              <?php if($f->status_fasilitas=='Publish'): ?>
            ...
            <td class="text-center" style="font-size:var(--font-xs);"><?= esc($f->hits) ?></td>
            <td>
              <div class="d-flex gap-1">
                <a href="<?= base_url('admin/fasilitas/edit/'.$f->id_fasilitas) ?>" class="btn btn-primary-action btn-sm" title="Edit">
                  <i class="fas fa-edit"></i>
                </a>
                <a href="<?= base_url('admin/fasilitas/delete/'.$f->id_fasilitas) ?>" class="btn btn-danger-action btn-sm delete-link" title="Hapus">
