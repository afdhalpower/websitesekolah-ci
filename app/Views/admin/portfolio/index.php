...
<div class="page-header-modern mb-4">
  <div>
    <h1 class="page-title"><i class="fas fa-briefcase"></i> Portfolio</h1>
    <p class="page-subtitle">Kelola portfolio gambar dan media</p>
  </div>
...
<!-- Bulk Action Form -->
<?= form_open(base_url('admin/portfolio/proses')) ?>
  ...
  <div class="card-modern-body" style="padding:0;">
    <div class="table-responsive">
      <table class="table-modern" id="example3">
        ...
        <tbody>
          <?php $no=1; foreach($portfolio as $p): ?>
          <tr>
            <td class="text-center">
              <input type="checkbox" name="id_portfolio[]" value="<?= esc($p->id_portfolio) ?>"
            ...
            <td>
              <a href="<?= base_url('admin/portfolio/edit/'.$p->id_portfolio) ?>" style="font-weight:500;color:var(--text);text-decoration:none;">
                <?= esc($p->judul_portfolio) ?>
              </a>
              <div style="font-size:var(--font-xs);color:var(--muted);margin-top:2px;">
                <i class="fas fa-home"></i> <?= esc($p->jenis_portfolio) ?>
                &nbsp;<i class="fas fa-eye"></i> <?= esc($p->hits) ?>
              </div>
            </td>
            <td>
              <span class="status-badge status-info" style="font-size:var(--font-xs);">
                <?= esc($p->nama_kategori_portfolio) ?>
              </span>
            </td>
            <td>
              <?php if($p->status_portfolio=='Publish'): ?>
            ...
            <td>
              <div class="d-flex gap-1">
                <a href="<?= base_url('admin/portfolio/edit/'.$p->id_portfolio) ?>" class="btn btn-primary-action btn-sm" title="Edit">
                  <i class="fas fa-edit"></i>
                </a>
                <a href="<?= base_url('admin/portfolio/delete/'.$p->id_portfolio) ?>" class="btn btn-danger-action btn-sm delete-link" title="Hapus">
